...
protected:
    ...
    string gender;
public: 
    Person(); Person(const Person&); int getYearOld();
...
int main() {
    ...
    while (number--) {
        int n; cin >> n;
        if (n == 1) {
            Employee x; cin >> x; a.push_back(x); cout << x;
        } else {
            Employer y; cin >> y; b.push_back(y); cout << y;
        }
    } 

    Employer::maxSalaryPerson(b);
    Employee::minDealPerson(a);
    return 0;
}
